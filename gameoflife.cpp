...
void ResetWithPattern(Pattern p)
{
  ...
  bool OnUserCreate() override {
    int width = 80;
    int height = 20;
    buttons.push_back({0,ScreenHeight() - height, width, height, "Random", true, Pattern::random});
    buttons.push_back({width,ScreenHeight() - height, width, height, "Gun", false, Pattern::gun});
    buttons.push_back({width * 2 ,ScreenHeight() - height, width, height, "Infinite", false, Pattern::infinite});
    buttons.push_back({width * 3,ScreenHeight() - height, width, height, "Pentomino", false, Pattern::pentomino});
  ...
  bool OnUserUpdate(float fElapsedTime) override {
    ...
    auto IsPointInButton = [&](int x, int y, Button b) {return (x > b.x && x < b.x + b.w && y > b.y && y < b.y + b.h);};
